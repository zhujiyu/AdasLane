...
struct cvtYUV422ToYellowGrayInvoker : cvtYellowGrayInvoker
{
    ...
    void operator()(const Range& range) const
    {
    	const int sh = src.rows>>1, sw = src.cols;
        const int rowStep = sh / dst->rows, colStep = sw / dst->cols, bgn = range.start*rowStep;

    	int ysd = src.step.p[0], uvsd = ysd>>1;

        const int ustart = sh * ysd, vstart = ustart + sh * uvsd;
        const uchar* ysrc = src.data + bgn * ysd;
        const uchar* usrc = src.data + ustart + bgn* uvsd;
        const uchar* vsrc = src.data + vstart + bgn* uvsd;
        ...
        if( colStep == 1 )
        {
			for( int row = range.start; row < range.end; row++ )
			{
				...
				for( int c = 0; c < dst->cols; c += 2, dd += 2 )
				{
					...
					int u = int(usrc[c>>1]) - 128, v = int(vsrc[c>>1]) - 128;
        ...
        else if( colStep == 2 )
        {
			for( int row = range.start; row < range.end; row++ )
			{
				...
				for( int c = 0; c < dst->cols; c++, dd++ )
				{
					...
					int u = int(usrc[c]) - 128, v = int(vsrc[c]) - 128;
        ...
        else if( colStep == 4 )
        {
			for( int row = range.start; row < range.end; row++ )
			{
				...
				for( int c = 0; c < dst->cols; c++, dd++ )
				{
					...
					int u = int(usrc[sc0]) - 128, v = int(vsrc[sc0]) - 128;
					int y00 = MAX(0, int(ysrc[sc1 + 0]) - 16) * ITUR_BT_601_CY;
...
{
	if( gray.empty() )
		gray.create(src.rows/2, src.cols, CV_8UC1);

	cvtYUV422ToYellowGrayInvoker cvt(src, &gray);

    if( gray.cols * gray.rows >= MIN_SIZE_FOR_PARALLEL_YUV422_CONVERSION)
...
void cvtYUV422toYellowGray(const cv::Mat src, cv::Mat &gray, const cv::Rect rect, cv::Mat &subsrc)
{
	...
	cvtYUV422toYellowGray(subsrc, gray);
}

void cvtYUV422toBGR(const cv::Mat src, cv::Mat &bgr)
{
	if( bgr.empty() )
		bgr.create(src.rows/2, src.cols, CV_8UC3);

	YUV422toRGB888Invoker cvt(bgr.data, bgr.step.p[0],
			src.data, src.step.p[0], src.cols, src.rows/2);

    if( bgr.cols * bgr.rows >= MIN_SIZE_FOR_PARALLEL_YUV422_CONVERSION)
...
//	if( bgr.empty() )
//		bgr.create(rect.size(), CV_8UC3);
//
//	YUV422toRGB888Invoker cvt(bgr.data, bgr.step.p[0],
//			subsrc.data, subsrc.step.p[0], subsrc.cols, subsrc.rows/2);
//
//    if( rect.width * rect.height >= MIN_SIZE_FOR_PARALLEL_YUV422_CONVERSION)
//        parallel_for_(Range(0, bgr.rows), cvt);
//    else
